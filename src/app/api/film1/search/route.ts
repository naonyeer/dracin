...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const rankResponse = await fetchCutad<{ data?: { sections?: any[] } }>("film1", "rank", { page: 1 });
    ...
    console.error("Film1 search error:", error);
    return createCutadErrorResponse(error, "Failed to fetch Film1 search");
  }
}
