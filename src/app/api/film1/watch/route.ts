...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetchCutad<{ data?: any }>("film1", "watch", { id: targetId });
    return encryptedResponse(buildCatalogWatchPayload(response.data));
  } catch (error) {
    console.error("Film1 watch error:", error);
    return createCutadErrorResponse(error, "Failed to fetch Film1 stream");
  }
}
