...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetchCutad<{ data?: any }>("film1", "detail", { id: bookId });
    return encryptedResponse(buildCatalogDetail(bookId, response.data));
  } catch (error) {
    console.error("Film1 detail error:", error);
    return createCutadErrorResponse(error, "Failed to fetch Film1 detail");
  }
}
