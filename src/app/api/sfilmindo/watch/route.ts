...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetchCutad<{ data?: any }>("sfilmindo", "watch", { id: targetId });
    return encryptedResponse(buildCatalogWatchPayload(response.data));
  } catch (error) {
    console.error("SFilmIndo watch error:", error);
    return createCutadErrorResponse(error, "Failed to fetch SFilmIndo stream");
  }
}
