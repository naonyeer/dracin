...
export async function GET(request: NextRequest) {
  ...
  try {
    try {
      const searchResponse = await fetchCutad<{ data?: { sections?: any[] } }>("moviebox", "search", { query });
    ...
    const rankResponse = await fetchCutad<{ data?: { sections?: any[] } }>("moviebox", "rank", { page: 1 });
    ...
    return createCutadErrorResponse(error, "Failed to fetch MovieBox search");
  }
}
